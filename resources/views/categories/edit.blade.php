                ...
                <div class="card-block">
                    ...
                    <section class="example">
                        <form action="{{ route('categories.edit', ['id' => $category->id]) }}"
                            ...
                            @if ($category->parent != null)
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control boxed" id="parent_id"
                                           value="{{ $category->parent->name }}" disabled>
                                </div>
                            @endif
                            <div class="form-group{{ $errors->has('country_id')  ? ' has-error' : '' }}">
                                <label for="country_id">Country</label>
                                <select name="country_id" id="country_id"
                                        class="form-control boxed"{{ $category->parent != null ? ' disabled' : '' }}>
                                    @if ($category->parent != null)
                                        <option value="{{ $category->parent->country->id }}">{{ $category->parent->country->name }}</option>
                                    @else
                                        @foreach ($countries as $country)
                                            <option value="{{ $country->id }}"{{ old('country_id', $category->country_id) == $country->id ? ' selected' : '' }}>{{ $country->name }}</option>
                                        @endforeach
                                    @endif
                                </select>
                            ...
                            <div class="form-group{{ $errors->has('type') ? ' has-error' : '' }}">
                                ...
                                <select name="type" id="type" class="form-control boxed">
                                    @foreach (\App\Models\Category::getCategories(true) as $type => $name)
                                        <option value="{{ $type }}"{{ old('type', $category->type) == $type ? ' selected' : '' }}>{{ $name }}</option>
                                    @endforeach
                                </select>
                                @if ($errors->has('type'))
                            ...
                            <textarea class="hidden" name="data"
                                      id="data">{!! old('data', $category->type != \App\Models\Category::TYPE_WEB_IMAGE ? $category->data : '') !!}</textarea>
                            ...
                            <div class="form-group hidden{{ $errors->has('data_image') ? ' has-error' : '' }}">
                                @if ($category->type == \App\Models\Category::TYPE_WEB_IMAGE)
                                    <div class="img-responsive">
                                        <img src="{{ asset($category->data) }}" class="img-thumbnail">
                                    </div>
                                @endif
